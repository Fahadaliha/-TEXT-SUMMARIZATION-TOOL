...
def summarize(text,size):
    ...
    sentence_score = {} #setence score for imp key words
    ...
    if size==1:
        numb=max(8,len(sentences)//10)
    elif size==2:
        numb=max(16,len(sentences)//4)
    else:
        numb=max(12,len(sentences)//6) #here we set how much we want summary in size
